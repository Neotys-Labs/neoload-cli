...
class TestDelete:
    def test_minimal(self, monkeypatch):
        ...
        mock_api_delete(monkeypatch, 'v2/tests/%s' % json_result['id'], '{"id":"%s"}' % json_result['id'])
    ...
    def test_with_name(self, monkeypatch):
        ...
        mock_api_delete(monkeypatch, 'v2/tests/%s' % json_result['id'],
    ...
    def test_not_found(self, monkeypatch, invalid_data):
        runner = CliRunner()
        mock_api_delete(monkeypatch, 'v2/tests/%s' % invalid_data.uuid, '{"code":"404", "message": "Test not found."}')
