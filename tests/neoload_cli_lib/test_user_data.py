...
class TestUserData:
    def test_login(self):
        ...
        assert login.token == user_data.get_login().token
        assert login.url == user_data.get_login().url
        assert user_data.get_login().token == 'abcdefghf201df15d897e7afe99a24159da764c7cc82b2ab'
        assert user_data.get_login().url == 'https://preprod-neoload.saas.neotys.com/'
