...
```
 ...
 - `-e, --engine-jar, --jar <path-or-url>` skips the latest-JAR download and runs a local patched file (hotfix) or an explicit download URL. If omitted, the CLI detects the OS, downloads the latest CheckVU JAR, and keeps a single cached copy.
